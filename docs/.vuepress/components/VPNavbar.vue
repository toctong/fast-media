...
useUpdateDeviceStatus(
  ...
  (mobileDesktopBreakpoint: number): void => {
    ...
    const brandWidth = navbarBrand.value?.offsetWidth ?? 0
    linksWrapperMaxWidth.value =
      window.innerWidth < mobileDesktopBreakpoint
        ? 0
        : navbar.value!.offsetWidth - navbarHorizontalPadding - brandWidth
  },
)
</script>
...
<template>
  <header ref="navbar" class="vp-navbar" vp-navbar>
    ...
    <span ref="navbar-brand" class="fm-navbar-brand">
      <VPNavbarBrand />
      <SearchBox />
    </span>
